...
<head>
    ...
    <title>Create Article — UNAISOC</title>
    <!-- Favicon -->
...
<!-- Mobile Sticky Top Bar (Mobile Create Article) -->
<header class="md:hidden sticky top-0 z-40 h-12 px-4 bg-white border-b border-zinc-300 flex justify-between items-center w-full shadow-xs">
  ...
  <div class="text-neutral-800 text-base font-extrabold font-['Geist']">New Article</div>
  <button type="button" onclick="submitMobileForm('publish')" class="px-3 py-1.5 bg-sky-500 hover:bg-sky-600 rounded-md text-white text-xs font-bold font-['Geist'] shadow-xs cursor-pointer">
    Publish
  </button>
</header>
...
<div class="md:hidden w-full min-h-screen bg-neutral-50 flex flex-col justify-start items-start pb-24">
  <form id="mobileForm" action="{{ route('articles.store') }}" method="POST" enctype="multipart/form-data" class="w-full p-4 flex flex-col justify-start items-start gap-5">
    @csrf
    <input type="hidden" id="mobileAction" name="action" value="publish">
    ...
    <div class="self-stretch flex flex-col justify-start items-start gap-1.5">
      ...
      <input type="text" name="title" value="{{ old('title') }}" required placeholder="e.g. The Future of Web Design" class="self-stretch px-3.5 py-3 bg-white rounded-lg outline outline-1 outline-offset-[-1px] outline-zinc-300 text-neutral-800 text-sm font-normal font-['Geist'] placeholder-neutral-400 focus:outline-sky-500">
    </div>
    ...
    <div class="self-stretch flex flex-col justify-start items-start gap-1.5">
      ...
      <div class="self-stretch px-3.5 py-2.5 bg-neutral-100 rounded-lg outline outline-1 outline-offset-[-1px] outline-zinc-300 inline-flex justify-start items-center gap-3">
        ...
        @endif
        <div class="flex-1 text-neutral-800 text-sm font-semibold font-['Geist']">{{ Auth::user()->name }} (You)</div>
    ...
    <div class="self-stretch flex flex-col justify-start items-start gap-1.5">
      <div class="text-neutral-500 text-xs font-bold font-['Geist'] uppercase">Article Body</div>
      <div class="self-stretch bg-white rounded-xl outline outline-1 outline-offset-[-1px] outline-zinc-300 flex flex-col justify-start items-start overflow-hidden">
        <div class="self-stretch px-3 py-2 bg-neutral-100 border-b border-zinc-300 inline-flex justify-start items-center gap-3 text-neutral-600">
          <button type="button" onclick="formatMobileDoc('bold')" class="font-bold text-xs hover:text-neutral-900 px-2 py-0.5 rounded hover:bg-neutral-200 transition" title="Bold">B</button>
          <button type="button" onclick="formatMobileDoc('italic')" class="italic text-xs hover:text-neutral-900 px-2 py-0.5 rounded hover:bg-neutral-200 transition" title="Italic">I</button>
          <button type="button" onclick="formatMobileDoc('underline')" class="underline text-xs hover:text-neutral-900 px-2 py-0.5 rounded hover:bg-neutral-200 transition" title="Underline">U</button>
          <div class="w-px h-3.5 bg-zinc-300"></div>
          <button type="button" onclick="formatMobileDoc('insertUnorderedList')" class="text-xs hover:text-neutral-900 px-2 py-0.5 rounded hover:bg-neutral-200 transition" title="Bullet List">• List</button>
          <button type="button" onclick="addMobileLink()" class="text-xs hover:text-neutral-900 px-2 py-0.5 rounded hover:bg-neutral-200 transition" title="Insert Link">🔗 Link</button>
        </div>
        <div id="mobileContentEditor" contenteditable="true" class="self-stretch min-h-[140px] p-4 text-neutral-800 text-sm font-normal font-['Geist'] leading-5 outline-none" placeholder="Write your thoughts here... Use markdown or the toolbar above to format.">{!! old('content') !!}</div>
        <textarea id="mobileHiddenContent" name="content" class="hidden" required>{{ old('content') }}</textarea>
      </div>
    </div>

    <!-- Bottom Actions -->
    <div class="self-stretch flex flex-col gap-2 pt-2">
      <button type="button" onclick="submitMobileForm('publish')" class="self-stretch p-3.5 bg-sky-500 hover:bg-sky-600 rounded-lg text-white text-base font-bold font-['Geist'] text-center cursor-pointer transition shadow-sm">
        Publish Article
      </button>
      
      <button type="button" onclick="submitMobileForm('draft')" class="self-stretch py-2 text-neutral-500 hover:text-neutral-800 text-xs font-semibold font-['Geist'] text-center cursor-pointer transition">
        Saved as Draft
      </button>
    </div>
    ...
    <div class="self-stretch flex flex-col justify-start items-start gap-9">
      ...
      <nav class="self-stretch flex flex-col justify-start items-start gap-2">
        ...
        <a href="{{ route('articles.create') }}" class="self-stretch p-3 bg-neutral-100 rounded-lg inline-flex justify-start items-center gap-4 transition">
          ...
          <div class="justify-start text-neutral-800 text-base font-bold font-['Geist']">Create</div>
        </a>
  ...
  <main class="flex-1 self-stretch bg-neutral-900/40 min-h-screen inline-flex flex-col justify-center items-center py-10 px-4 overflow-y-auto">
    <div class="w-[840px] max-w-full bg-white rounded-2xl shadow-[0px_12px_32px_0px_rgba(0,0,0,0.10)] flex flex-col justify-start items-start overflow-hidden">
      ...
      <div class="self-stretch px-6 py-4 border-b border-zinc-300 inline-flex justify-between items-center">
        <div class="justify-start text-neutral-800 text-base font-bold font-['Geist']">Create New Article</div>
      ...
      <form action="{{ route('articles.store') }}" method="POST" enctype="multipart/form-data" class="self-stretch flex-1 flex flex-row justify-start items-start">
        @csrf
...
<script>
  ...
  function formatMobileDoc(command) {
    document.execCommand(command, false, null);
    document.getElementById('mobileContentEditor').focus();
    syncMobileContent();
  }
  ...
  function syncMobileContent() {
    const editor = document.getElementById('mobileContentEditor');
    const textarea = document.getElementById('mobileHiddenContent');
    textarea.value = editor.innerHTML.trim();
  }

  if (document.getElementById('mobileContentEditor')) {
    document.getElementById('mobileContentEditor').addEventListener('input', syncMobileContent);
  }

  function previewMobileFile(input) {
    if (input.files && input.files[0]) {
      const reader = new FileReader();
      reader.onload = function(e) {
